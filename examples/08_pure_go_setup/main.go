...
func main() {
	...
	evaluator := baccess.NewEvaluator[User, Data]()
	rbac := baccess.NewRBAC[User, Data]()
	...
	req1 := baccess.AccessRequest[User, Data]{Subject: admin, Resource: data, Action: "delete"}
	fmt.Printf("Admin delete: %v\n", evaluator.Evaluate(req1))

	req2 := baccess.AccessRequest[User, Data]{Subject: user, Resource: data, Action: "delete"}
	fmt.Printf("User delete own: %v\n", evaluator.Evaluate(req2))

	data2 := Data{OwnerID: "other"}
	req3 := baccess.AccessRequest[User, Data]{Subject: user, Resource: data2, Action: "delete"}
